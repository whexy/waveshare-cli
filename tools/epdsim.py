...
class Simulator:
    ...
    def command(self, command, seq, data):
        ...
        def ack(payload=b''):
            return make_frame(ACK, seq, payload)

        def nak(code):
            return make_frame(NAK, seq, bytes([code]))
            ...
            if command == 5:
                return ack(
                    struct.pack(
                        ...
                        min(self.partials, 65535),
                        int((self.clock() - self.last_full) * 1000) & 0xFFFFFFFF,
                        self.bbox is not None,
                        *(self.bbox or (0, 0, 0, 0)),
                    )
                )
            return ack()
        if command == 4:
        ...
        if command == 19:
            ...
            if not w or not h or x + w > 100 or y + h > 480:
                return nak(3)
            if len(data) != 8 + w * h:
                return nak(1)
            for r in range(h):
                self.framebuffer[(y + r) * 100 + x : (y + r) * 100 + x + w] = data[
                    8 + r * w : 8 + (r + 1) * w
                ]
