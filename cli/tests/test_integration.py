...
class IntegrationTests(unittest.TestCase):
    def test_console_simulator(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            sim = subprocess.Popen([sys.executable,str(ROOT/'tools/epdsim.py'),
                                    '--output',str(output)],stdout=subprocess.PIPE,text=True)
            try:
                port = sim.stdout.readline().strip()
                self.assertTrue(port.startswith('/dev/ttys') or port.startswith('/dev/pts/'), port)
                def run(*args, data=None):
                    result = subprocess.run([sys.executable,'-m','epaper','--port',port,*args],
                ...
                sim.terminate(); sim.wait(timeout=5); sim.stdout.close()
